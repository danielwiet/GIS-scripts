...
# iterates through each folder in the path
for folders in os.listdir(path):
	...
	if not folders.startswith('.'):
		...
		# create destination directory on the desktop if it does not exist
		if not os.path.exists(desktop + '/zipped_' + folders + '_shapefiles'):
			os.makedirs(desktop + '/zipped_' + folders + '_shapefiles')
		...
		for f in ziplist:
			...
			# creates the name for each zipefile based on the shapefile root names on the desktop folder
			file_name = os.path.join(desktop + '/zipped_' + folders + '_shapefiles', f+'.zip')
